...
#define SHSTRTAB "\0.shstrtab\0.symtab\0.strtab\0.text"
#define STRTAB "\0cont"
...
typedef struct {
    ...
    __attribute__((aligned(4))) m4_elf_content_t cont;
} m4_elf_nuttx_t;

int m4_elf_nuttx_size(void)
{
    return offsetof(m4_elf_nuttx_t, cont) + sizeof(m4_elf_content_t);
}
...
void m4_elf_nuttx(void * aligned_elf_dst, m4_arch_t machine, int bin_len)
{
    ...
    e->phdrs[0].p_vaddr = 0;
    e->phdrs[0].p_paddr = 0;
    e->phdrs[0].p_filesz = sizeof(m4_elf_content_t) + bin_len;
    e->phdrs[0].p_memsz = sizeof(m4_elf_content_t) + bin_len;
    e->phdrs[0].p_flags = PF_X | PF_R;
    e->phdrs[0].p_align = 4;
